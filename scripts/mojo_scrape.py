...
for m in movie_links:
    print(count)
    time.sleep(0.025+np.random.random()/50)
    count += 1
    ...
    if rel_date < datetime.datetime(2000, 1, 1):
        print(rel_date)
        continue
    elif ('N/A' == budget) or (budget is None):
        print('b')
        continue
    elif ('N/A' == gross) or (gross is None):
        print('$')
        continue
        ...
        mojo_data['budget'].append(budget.replace('$', '').replace(',', ''))

        mojo_data['title'].append(soup.find('title').text.split('(')[0].strip())
        mojo_data['MPAA'].append(get_movie_value(soup, 'MPAA'))
        mojo_data['Dist'].append(get_movie_value(soup, 'Distributor'))
